...
const SUBJECTS = {
  os: {
    ...
    pools:   typeof STAGE_POOLS !== 'undefined' ? STAGE_POOLS : {},
    stages: [
  ...
  hist: {
    ...
    pools:   typeof HIST_STAGE_POOLS !== 'undefined' ? HIST_STAGE_POOLS : {},
    stages: [
...
function startGame(stage) {
  ...
  // Re-bind pools in case they loaded after this script
  if (_currentSubject === 'os') {
    subj.pools = typeof STAGE_POOLS !== 'undefined' ? STAGE_POOLS : {};
  } else {
    subj.pools = typeof HIST_STAGE_POOLS !== 'undefined' ? HIST_STAGE_POOLS : {};
  }

  const pool = subj.pools[stage] || [];
